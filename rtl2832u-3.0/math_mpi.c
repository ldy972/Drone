	...
	)
{
	...
	for(i = 0; i < MPI_LONG_BYTE_NUM; i++)
		pMpiVar->Value[i] = (u8)((Value >> (MPI_BYTE_SHIFT * i)) & MPI_BYTE_MASK);

	
	// Get extension byte according to signed bit.
	SignedBit = (u8)((Value >> (MPI_LONG_BIT_NUM - 1)) & MPI_BIT_0_MASK);
...
Use MpiGetValue() to get multiple precision u32eger MPI value.
...
*/
void MpiGetValue(MPI MpiVar, s64* pValue)
{
	s32 i;
	u64 Value;
...
{
	u64 TargetBytePos, TargetBitPos;
	...
	pMpiVar->Value[TargetBytePos] &= (u8)(~(0x1 << TargetBitPos));
...
Use MpiGetBit() to get multiple precision u32eger MPI bit value.
...
Use MpiGetBit() to get multiple precision u32eger MPI signed bit value.
	...
	)
{
	...
	for(i = 0; i < MPI_VALUE_BYTE_NUM_MAX; i++)
	{
		...
		pSum->Value[i] = (u8)(MiddleResult & MPI_BYTE_MASK);
		Carry          = (u8)((MiddleResult >> MPI_BYTE_SHIFT) & MPI_BYTE_MASK);
	}
	...
	)
{
	...
	u8 MultiplicandSignedBit, MultiplicatorSignedBit;
	...
	)
{
	...
	u8 DividendSignedBit, DivisorSignedBit;
	MPI DividendAbs, DivisorAbs;

	u64 PrimaryDividendBitNum;
	u8 ShiftBit;
...
@brief   Compare multiple precision signed integers with equal-to criterion.

Use MpiEqualTo() to compare multiple precision signed integers with equal-to criterion.
...
@brief   Compare multiple precision signed integers with greater-than criterion.

Use MpiGreaterThan() to compare multiple precision signed integers with greater-than criterion.
...
@brief   Compare multiple precision signed integers with less-than criterion.

Use MpiLessThan() to compare multiple precision signed integers with less-than criterion.
...
*/
void MpiLog2(MPI *pResult, MPI MpiVar, u64 ResultFracBitNum)
{
	u32 i;
	MPI MiddleResult;
	u8 BitValue;
	...
	MpiSetValue(pResult, (s64)(MpiVar.BitNum - 2));
