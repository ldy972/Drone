	...
	)
{
	...
	CrystalFreqKhz = (u32)((pExtra->CrystalFreqHz + 500) / 1000);
	...
	)
{
	...
	RfFreqKhz = (u32)((RfFreqHz + 500) / 1000);
	CrystalFreqKhz = (u32)((pExtra->CrystalFreqHz + 500) / 1000);
	...
	)
{
	...
	if(fc2580_set_filter(pTuner, (u8)BandwidthMode, CrystalFreqKhz) != FC2580_FCI_SUCCESS)
...
==============================================================================*/
void fc2580_wait_msec(TUNER_MODULE *pTuner, s32 a)
{
	...
	pBaseInterface->WaitMs(pBaseInterface, (u64)a);
...
==============================================================================*/
fc2580_fci_result_type fc2580_i2c_write( TUNER_MODULE *pTuner, u8 addr, u8 data )
{
...
==============================================================================*/
fc2580_fci_result_type fc2580_i2c_read( TUNER_MODULE *pTuner, u8 addr, u8 *read_data )
{
	I2C_BRIDGE_MODULE *pI2cBridge;
	u8 DeviceAddr;
...
==============================================================================*/
fc2580_fci_result_type fc2580_set_init( TUNER_MODULE *pTuner, s32 ifagc_mode, u32 freq_xtal )
{
...
==============================================================================*/
fc2580_fci_result_type fc2580_set_freq( TUNER_MODULE *pTuner, u32 f_lo, u32 freq_xtal )
{
	u32 f_diff, f_diff_shifted, n_val, k_val;
	u32 f_vco, r_val, f_comp;
	u8 pre_shift_bits = 4;// number of preshift to prevent overflow in shifting f_diff to f_diff_shifted
	u8 data_0x18;
	u8 data_0x02 = (USE_EXT_CLK<<5)|0x0E;
...
//	}
	...
	data_0x18 = ( ( r_val == 1 )? 0x00 : ( ( r_val == 2 )? 0x10 : 0x20 ) ) + (u8)(k_val >> 16);
	result &= fc2580_i2c_write(pTuner, 0x18, data_0x18);						//Load 'R' value and high part of 'K' values
	result &= fc2580_i2c_write(pTuner, 0x1A, (u8)( k_val >> 8 ) );	//Load middle part of 'K' value
	result &= fc2580_i2c_write(pTuner, 0x1B, (u8)( k_val ) );		//Load lower part of 'K' value
	result &= fc2580_i2c_write(pTuner, 0x1C, (u8)( n_val ) );		//Load 'N' value

	//A command about UHF LNA Load Cap
	if( band == FC2580_UHF_BAND )
		result &= fc2580_i2c_write(pTuner, 0x2D, ( f_lo <= (u32)794000 )? 0x9F : 0x8F );	//LNA_OUT_CAP
...
==============================================================================*/
fc2580_fci_result_type fc2580_set_filter( TUNER_MODULE *pTuner, u8 filter_bw, u32 freq_xtal )
{
	u8	cal_mon, i;
	fc2580_fci_result_type result = FC2580_FCI_SUCCESS;
	...
	if(filter_bw == 1)
	{
		...
		result &= fc2580_i2c_write(pTuner, 0x37, (u8)(4151*freq_xtal/1000000) );
	...
	if(filter_bw == 6)
	{
		...
		result &= fc2580_i2c_write(pTuner, 0x37, (u8)(4400*freq_xtal/1000000) );
		result &= fc2580_i2c_write(pTuner, 0x39, 0x00);
		result &= fc2580_i2c_write(pTuner, 0x2E, 0x09);
	}
	else if(filter_bw == 7)
	{
		result &= fc2580_i2c_write(pTuner, 0x36, 0x18);
		result &= fc2580_i2c_write(pTuner, 0x37, (u8)(3910*freq_xtal/1000000) );
	...
	{
		result &= fc2580_i2c_write(pTuner, 0x36, 0x18);
		result &= fc2580_i2c_write(pTuner, 0x37, (u8)(3300*freq_xtal/1000000) );
		result &= fc2580_i2c_write(pTuner, 0x39, 0x80);
		result &= fc2580_i2c_write(pTuner, 0x2E, 0x09);
	}
...
==============================================================================*/
//s32 fc2580_get_rssi(void) {
//	
//	u8 s_lna, s_rfvga, s_cfs, s_ifvga;
//	s32 ofs_lna, ofs_rfvga, ofs_csf, ofs_ifvga, rssi;
//
...
==============================================================================*/
//void fc2580_set_freq_xtal(u32 frequency) {
//
//	freq_xtal = frequency;
//
//}
