...
extern s32 demod_default_type;
extern s32 dtmb_error_packet_discard;
extern s32 dvb_use_rtl2832u_card_type;
extern s32 dvb_use_rtl2832u_rc_mode;
...
static s32 rtl2832_reg_mask[32]= {
    0x00000001,
    0x00000003,
    0x00000007,
    0x0000000f,
...
static s32 check_dtmb_support(struct rtl2832_state* p_state);
static s32 check_dvbc_support(struct rtl2832_state* p_state);
static s32 set_demod_2836_power(struct rtl2832_state* p_state, s32  onoff);
static s32 rtl2840_on_hwreset(struct rtl2832_state* p_state);
static s32 set_demod_2840_power(struct rtl2832_state* p_state, s32  onoff);
static s32 demod_init_setting(struct rtl2832_state* p_state);
static s32 build_nim_module(struct rtl2832_state* p_state);
static s32 rtl2836_scan_procedure(struct rtl2832_state* p_state);
static s32 fc0012_get_signal_strength(struct rtl2832_state* p_state, u64* strength);
static s32 rtl2832_sleep_mode(struct rtl2832_state* p_state);

static void custom_wait_ms(BASE_INTERFACE_MODULE*	pBaseInterface,
                           u64 WaitTimeMs)
{
...
static s32 custom_i2c_read(BASE_INTERFACE_MODULE* pBaseInterface,
                           u8 DeviceAddr,
                           u8* pReadingBytes,
                           u64 ByteNum)
{
	...
	if(read_rtl2832_stdi2c(d, DeviceAddr, pReadingBytes, ByteNum))
		goto error;
...
static s32 custom_i2c_write(BASE_INTERFACE_MODULE*	pBaseInterface,
                            u8 DeviceAddr,
                            ...
                            u64 ByteNum)
{
	...
	pBaseInterface->GetUserDefinedDataPointer(pBaseInterface, (void **)&d);
	if ( write_rtl2832_stdi2c( d, DeviceAddr , (u8*)pWritingBytes , ByteNum ) ) goto error;
...
static s32 read_usb_sys_register(struct rtl2832_state* p_state,
                                 rtl2832_reg_map_index reg_map_index,
                                 s32* p_val)
{
	RegType reg_type = rtl2832_reg_map[reg_map_index].reg_type;
	u16     reg_addr = rtl2832_reg_map[reg_map_index].reg_addr;
	s32     bit_low  = rtl2832_reg_map[reg_map_index].bit_low;
	s32     bit_high = rtl2832_reg_map[reg_map_index].bit_high;

	s32     n_byte_read = (bit_high >> 3) + 1;

	*p_val= 0;

	if(read_usb_sys_int_bytes(p_state->d, reg_type, reg_addr, n_byte_read, p_val))
		goto error;
...
static s32 write_usb_sys_register(struct rtl2832_state* p_state,
                                  rtl2832_reg_map_index reg_map_index,
                                  s32 val_write)
{
	RegType reg_type = rtl2832_reg_map[reg_map_index].reg_type;
	...
	s32 n_byte_write = (bit_high >> 3) + 1;
	s32 val_read = 0;
	s32 new_val_write;

	if (read_usb_sys_int_bytes(p_state->d, reg_type, reg_addr, n_byte_write, &val_read))
		goto error;

	new_val_write = (val_read & (~(rtl2832_reg_mask[bit_high- bit_low]<< bit_low))) | (val_write<< bit_low);

	if (write_usb_sys_int_bytes(p_state->d, reg_type, reg_addr, n_byte_write, new_val_write))
		goto error;
...
static s32 max3543_set_power(struct rtl2832_state* p_state, u8 onoff)
{
	u8	data;
	u8	i2c_repeater;	
...
static s32 set_tuner_power(struct rtl2832_state* p_state, u8 b_gpio4, u8 onoff)
{
	s32 data;

	if(onoff == 0)
		max3543_set_power(p_state, onoff);

	deb_info(" +%s \n", __FUNCTION__);
	
	if ( read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_GPIO_OUTPUT_VAL, &data))
		goto error;
	...
	if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_GPIO_OUTPUT_VAL,data))
		goto error;
...
static s32 set_demod_power(struct rtl2832_state* p_state, u8 onoff)
{
	s32 data;
...
static s32 gpio3_out_setting(struct rtl2832_state* p_state)
{
	s32 data;
	...
	data = 2;
	if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_GPIO_CFG0_BIT67,data))
		goto error;
	...
	data = 0;
	if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_GPIO_DIR_BIT3,data))
		goto error;

	// GPOE_GPIO3=1, GPIO3 output enable
	data = 1;
	if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_GPIO_OUTPUT_EN_BIT3,data))
		goto error;

	//BTN_WAKEUP_DIS = 1
	data = 1;
	if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_CTRL_BIT5,data))
		goto error;
...
static s32 usb_epa_fifo_reset(struct rtl2832_state* p_state)
{
	s32 data;
	...
	data = 0x0210;
	if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_EPA_CTL,data))
		goto error;

	data = 0xffff;
	if(read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_EPA_CTL,&data))
		goto error;
...
static s32 usb_init_bulk_setting(struct rtl2832_state* p_state)
{
	s32 data;
	...
	//3 2.DMA enable.
	if(read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_SYSCTL, &data))
		goto error;

	data &=0xffffff00;
	data |= 0x09;
	if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_SYSCTL, data))
		goto error;
	...
	data = 0;
	if(read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_EPA_CFG, &data))
		goto error;
	...
	if(read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_STAT, &data))
		goto error;

	if(data&0x01)  
	{
		data = 0x00000200;
		if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_EPA_MAXPKT, data))
			goto error;

		data=0;
		if(read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_EPA_MAXPKT, &data))
			goto error;
...
static s32 usb_init_setting(struct rtl2832_state* p_state)
{
	s32 data;

	deb_info(" +%s \n", __FUNCTION__);

	if(usb_init_bulk_setting(p_state))
		goto error;

	//3 change fifo length of EPA 
	data = 0x00000014;
	if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_EPA_FIFO_CFG, data))
		goto error;

	data = 0xcccccccc;
	if(read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_USB_EPA_FIFO_CFG, &data))
		goto error;
...
static s32 suspend_latch_setting(struct rtl2832_state* p_state, u8 resume)
{
	s32 data;
	deb_info(" +%s \n", __FUNCTION__);
	...
	if(resume)
	{
		...
		if(read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_DEMOD_CTL1, &data))
			goto error;

		data &= (~BIT4);	//set bit4 to 0

		if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_DEMOD_CTL1,data))
			goto error;
	}
	else
	{
...
static s32 demod_ctl1_setting(struct rtl2832_state* p_state, u8 resume)
{
	s32 data;
...
static s32 demod_ctl_setting(struct rtl2832_state* p_state, u8 resume, u8 on)
{
	s32 data;
	u8  tmp;
	...
	{
		// PLL setting
		if(read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_DEMOD_CTL, &data))
			goto error;

		data |= BIT7;		//set bit7 to 1

		if(write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_DEMOD_CTL,data))
			goto error;
		...
		if(read_demod_register(p_state->d, RTL2832_DEMOD_ADDR, 0, 1, &tmp, 1))
			goto error;
		//2 -End LOCK
...
static s32 read_tuner_id_register(struct rtl2832_state*	p_state,
                                  u8  tuner_addr,
                                  u8  tuner_offset,
                                  u8* id_data,
                                  u8  length)
...
static s32 check_mxl5007t_chip_version(struct rtl2832_state* p_state, u8 *chipversion)
{
	u8 Buffer[LEN_2_BYTE];
	u8	i2c_repeater;	

	struct dvb_usb_device*	d = p_state->d;	

	Buffer[0] = (u8)MXL5007T_I2C_READING_CONST;
	Buffer[1] = (u8)MXL5007T_CHECK_ADDRESS;
...
static s32 check_tuner_type(struct rtl2832_state *p_state)
{
	u8 tuner_id_data[2];
	u8 chip_version;
...
static s32 gpio1_output_enable_direction(struct rtl2832_state*	p_state)
{
	s32 data;
...
static s32 gpio6_output_enable_direction(struct rtl2832_state* p_state)
{
	s32 data;
...
static s32 gpio5_output_enable_direction(struct rtl2832_state* p_state)
{
	s32 data;
...
static s32 check_dvbt_reset_parameters(struct rtl2832_state* p_state,
                                       u64  frequency,
...
void rtl2832_update_functions(struct work_struct *work)
{
...
void rtl2836_update_function(struct work_struct *work)
{
	...
	deb_info("%s -- ***GetPer= %d***\n", __FUNCTION__, (s32)Per1);

	pDtmbDemod->GetSnrDb(pDtmbDemod,&Data1,&Data2);            
	deb_info("%s -- ***SNR = %d***\n",__FUNCTION__, (s32)(Data1>>2));
...
static s32 rtl2832_init(struct dvb_frontend*	fe)
{
	...
	if(usb_init_setting(p_state))
		goto error;
	
	if(gpio3_out_setting(p_state))     // Set GPIO3 OUT
 		goto error;

	if(demod_ctl1_setting(p_state, 1)) // DEMOD_CTL1, resume = 1
		goto error;

	if(dvb_use_rtl2832u_card_type)
	{
		if(set_demod_power(p_state, 1))
			goto error;                  // turn ON demod power
	}
	...
	if(p_state->demod_type == RTL2832)
	{
		...
		break;
			...
			if (frequency > 300000000)
			{
				...
				read_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_GPIO_OUTPUT_VAL, &s32_data);
				s32_data |= BIT6; // set GPIO6 high
				write_usb_sys_register(p_state, RTD2831_RMAP_INDEX_SYS_GPIO_OUTPUT_VAL, s32_data);	
	...
	fe_status_t*	status)
{
	...
	*status = 0;	//3initialize "status"
	...
	if(p_state->demod_type == RTL2832)
	{
		...
		deb_info("%s : ber_num = %d\n", __FUNCTION__, (u32)ber_num);	
		deb_info("%s : snr = %d \n", __FUNCTION__, (s32)snr);	
	}
...
static s32 fc0012_get_signal_strength(struct rtl2832_state	*p_state,u64 *strength)
{
	s32 s32Temp=0;
	s32 Power=0;
	s32 s32TotalAGCGain=0;
	s32 s32LNA=0;
	u8 ReadingByte=0;
	s32 LnaGain_reg=0;
	s32 NumberOfLnaGainTable=0;
	s32 i=0;
	s32 Index=0;
	TUNER_MODULE *pTuner=NULL;
	...
	if(FC0012_Read(pTuner, 0x12, &ReadingByte) != FUNCTION_SUCCESS)
		goto error;
	s32Temp=(s32)ReadingByte;

	if(FC0012_Read(pTuner, 0x13, &ReadingByte) != FUNCTION_SUCCESS)
		goto error;
	LnaGain_reg=(s32)ReadingByte&0x7f;
	...
	s32LNA=FC0012_LNA_GAIN_TABLE[Index].LnaGain;

	s32TotalAGCGain = ((abs((s32Temp >> 5) - 7) -2) * 2 + (s32Temp & 0x1F) * 2);

	 Power= INPUT_ADC_LEVEL - s32TotalAGCGain - (s32LNA/10);				

	deb_info(" %s power=%d form fc0012(%x,%x,%x)\n", __FUNCTION__,Power,s32Temp,LnaGain_reg,s32LNA);			
		...
		s32  onoff)
{
	...
	{
		//3 2. RTL2836 AGC = 1 
		if ( read_rtl2836_demod_register(p_state->d,  RTL2836_DEMOD_ADDR,  PAGE_0, 0x01, &dataByte,  LEN_1_BYTE)) goto error;
		dataByte |=BIT_2_MASK;
		dataByte &=(~BIT_3_MASK);
		if(write_rtl2836_demod_register(p_state->d,  RTL2836_DEMOD_ADDR,   PAGE_0, 0x01, &dataByte, LEN_1_BYTE))  goto error;	
	}
	else
	{
...
static s32 rtl2836_scan_procedure(struct rtl2832_state * p_state)
{
	...
	deb_info("%s Signal Present = 0x %x\n", __FUNCTION__, val);
