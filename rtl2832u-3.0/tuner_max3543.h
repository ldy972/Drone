...
//#else
	/* Macros used for scaling frequency constants.  */
	/* Use this form if NOT using floating point math. */
//		#define scalefrq(x)  ( (u16) ( ( (float) x ) * (float) LOSCALE ) ) 
//		#define scalextal(x) ( (u16) ( ( (float) x ) * (float) XTALSCALE ) )
...
typedef  s16 INT_16;          /* compiler type for 16 bit integer */
typedef  u16 UINT_16; /* compiler type for 16 bit unsigned integer */
typedef  u64 UINT_32;  /* compiler type for 32 bit unsigned integer */

typedef enum {IFOUT_1,IFOUT_2} outmd;

//s32 MAX3543_Init(TUNER_MODULE *pTuner, UINT_16 RfFreq);
s32 MAX3543_Init(TUNER_MODULE *pTuner);
s32 MAX3543_SetFrequency(TUNER_MODULE *pTuner, UINT_16 RF_Frequency);
//u16 MAX3543_Read(UINT_16 reg);
//void MAX3543_Write(UINT_16 reg, UINT_16 value);
s32 MAX3543_LockDetect(TUNER_MODULE *pTuner, s32 *pAnswer);
s32 MAX3543_Standard(TUNER_MODULE *pTuner, standard bcstd, outputmode outmd);
s32 MAX3543_ChannelBW(TUNER_MODULE *pTuner, UINT_16 bw);
s32 MAX3543_SetTrackingFilter(TUNER_MODULE *pTuner, UINT_16 RF_Frequency);
s32 MAX3543_ReadROM(TUNER_MODULE *pTuner);
UINT_16 tfs_i(UINT_16 S0, UINT_16 S1, UINT_16 FreqRF, const UINT_16 c[5]);
s32 MAX3543_SeedVCO(TUNER_MODULE *pTuner, UINT_16 Fvco);
...
//    void  Max3543_Write(u16 RegAddr, u16 data);
