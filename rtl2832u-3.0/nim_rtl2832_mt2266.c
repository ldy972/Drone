...
UData_t demod_get_pd(handle_t demod_handle, u16 *pd_value)
{
	DVBT_DEMOD_MODULE *pDemod;
	u64 RssiR;
...
UData_t demod_get_agc(handle_t demod_handle,
                      u16 *rf_level,
                      u16 *bb_level)
{
	DVBT_DEMOD_MODULE *pDemod;
	...
	*rf_level = (u16)((RfAgc + (1 << (RTL2832_RF_AGC_REG_BIT_NUM - 1))) *
		(1 << (MT2266_DEMOD_ASSUMED_AGC_REG_BIT_NUM - RTL2832_RF_AGC_REG_BIT_NUM)));

	*bb_level = (u16)((IfAgc + (1 << (RTL2832_IF_AGC_REG_BIT_NUM - 1))) *
...
UData_t demod_set_bbagclim(handle_t demod_handle,
                           s32 on_off_status)
{
	DVBT_DEMOD_MODULE *pDemod;
	u64 IfAgcMinBinary;
	...
	)
{
	...
	s32 DemodBandwidthMode;
	u32 TunerBandwidthHz;
	u32 TargetTunerBandwidthHz;
...
UData_t demod_pdcontrol_reset(handle_t demod_handle, handle_t tuner_handle, u8 *agc_current_state) {
...
UData_t demod_pdcontrol(handle_t demod_handle, handle_t tuner_handle, u8* lna_config, u8* uhf_sens,
					 u8 *agc_current_state, u64 *lna_gain_old)
 {
	u16 pd_value;
	u16 rf_level, bb_level;
	u64 lna_gain;
	u8 zin=0;
	u32 tmp_freq=0,tmp_lna_gain=0;
...
//	demod_data_t* local_data;
	...
	u16 sens_on[]  = {11479, 11479, 32763};
	u16 sens_off[] = {36867, 36867, 44767};
	u16 lin_off[]  = {23619, 23619, 23619};
	u16 lin_on[]   = {38355, 38355, 38355};
	u16 pd_upper[] = {85,    85,    85};
	u16 pd_lower[] = {74,    74,    74};
	u8 next_state;
