...
struct rtl2832_state {
	...
	struct mutex					i2c_repeater_mutex;
       ...
       u64					current_frequency;	
	enum fe_bandwidth			current_bandwidth;		
	   
	RTL2832_TUNER_TYPE			tuner_type;
	u8					is_mt2266_nim_module_built;  //3 For close MT handle
	u8					is_mt2063_nim_module_built;  //3 For close MT handle


	//3 DTMB related begin ---
	u32                                demod_support_type;
	u32                                demod_type;
	u32                                demod_ask_type;
...
s32 rtl2832_read_signal_quality(
