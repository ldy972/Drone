	...
	)
{
	...
	{
		u8 SpecReg0Sel;
		u8 SpecReg0ValueTable[RTL2840_SPEC_REG_0_VALUE_TABLE_LEN];
	}
	...
	u8 SpecReg0Sel;
	const u8 *pSpecReg0ValueTable;

	u64 QamSpecInitA2Backup;
	u64 RegValue, RegValueComparison;
	s32 AreAllValueEqual;
	...
	)
{
	typedef struct
	{
		...
		u8 Msb;
		u8 Lsb;
		u64 WritingValue[QAM_QAM_MODE_NUM];
	}
	QAM_MODE_REG_ENTRY;
	...
	{
		u8 SpecReg0Sel;
		u8 SpecReg0ValueTable[QAM_QAM_MODE_NUM][RTL2840_SPEC_REG_0_VALUE_TABLE_LEN];
	}
	...
	)
{
	typedef struct
	{
		u64 TrDeciRatioRangeMin;
		u8 SymbolRateReg0;
		u64 SymbolRateValue[RTL2840_SYMBOL_RATE_VALUE_TABLE_LEN];
	}
	SYMBOL_RATE_ENTRY;
	...
	MpiGetValue(MpiVar, (s64 *)&TrDeciRatio);
...
{
	static const u64 AlphaValueTable[QAM_ALPHA_MODE_NUM][RTL2840_ALPHA_VALUE_TABLE_LEN] =
	{
	...
	)
{
	...
	MpiGetValue(MpiVar, (s64 *)&DdcFreq);
...
{
	static const s8 SpecInvValueTable[SPECTRUM_MODE_NUM] =
	{
		// SpecInv
		0,				// Normal spectrum
		1,				// Inverse spectrum
	...
	)
{
	...
	MpiSetValue(&MpiSymbolRateHz,  (s64)SymbolRateHz);
	MpiSetValue(&MpiCrystalFreqHz, (s64)CrystalFreqHz);
	...
	)
{
	...
	u32  WaitCnt;
	s32 FrameLock;
	u64 BerReg2, BerReg2Msb, BerReg2Lsb;
	u64 BerReg0, BerReg1;
...
{
	static const u64 SnrConstTable[QAM_QAM_MODE_NUM] =
	{
	...
	)
{
	...
	u64 Buffer[QAM_MONITOR_REG_INFO_TABLE_LEN];
