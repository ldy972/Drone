...
extern s32 rtl2832_read_signal_quality(
	struct dvb_frontend*	fe,
	u32*	quality);
extern s32 
...
static s32 fe_rw_std_cmd_ioctrl(struct dvb_frontend *fe, void *parg)
{

	struct rtl2832u_ioctl_rwcmd_struct*  rw_ctrl = (struct rtl2832u_ioctl_rwcmd_struct*)parg;
	s32 ret =0;
	s32 rw_cmd = rw_ctrl->cmd_num;
...
static s32 fe_demod_info_ioctrl(struct dvb_frontend *fe, void *parg)
{
	...
	u32			quality=0;
	s32 			answer=0;
	u64 		BerNum=0,BerDen=1;
	s64 			SnrDbNum=0, SnrDbDen=1;
	s64 			tr_ppm=0,cr_hz=0;
	u64 		data_length=info_ctrl->data_length;
	s32 			info_cmd = info_ctrl->cmd_num;
	deb_info("+%s : ioctl -demod info cmd = %d ,data length= %d (must > %d )\n", __FUNCTION__,info_cmd,(s32)data_length,INFO_DEMOD_DATA_LEN);
	if (p_state->demod_type != RTL2832){
		deb_info("ERROR::demod type not rtl2832u...\n");
		return -1;
	}
	if (data_length < INFO_DEMOD_DATA_LEN){
		deb_info("ERROR::data length is too s16...,must be greater than %d \n",INFO_DEMOD_DATA_LEN);
		return -1;
	}
	pNim = p_state->pNim;	
	if (pNim == NULL){
	...
	switch(info_cmd)
	{
		case FE_GET_SIGN_STRENGTH:
			...
			{
				ret=10;
				info_ctrl->data[INFO_DATA_VALUE]=(u64)strength;
				info_ctrl->data[INFO_DATA_DEN]=1;
//				deb_info("rtl2832u ioctl (FE_GET_SIGN_STRENGTH)- strength = %x (%x). \n",strength,info_ctrl->data[INFO_DATA_VALUE] );
		...
		case FE_GET_SIGN_QUALITY:
			...
			{
				ret=10;
				info_ctrl->data[INFO_DATA_VALUE]=(u64)quality;
				info_ctrl->data[INFO_DATA_DEN]=1;
...
//	deb_info("-%s :ioctl - std info cmd ret= %x,data=%x,den=%x\n", __FUNCTION__,ret,(s32)info_ctrl->data[INFO_DATA_VALUE],info_ctrl->data[INFO_DATA_DEN]);
	...
	u16			bytelength)
{
	s32 i=0;
	s32 ret = -1;
	u8 data_temp[MAX_STD_I2C_BUF_NUM];  
	struct rtl2832_state*	p_state = fe->demodulator_priv;
