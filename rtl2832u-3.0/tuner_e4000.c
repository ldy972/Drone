	...
	)
{
	...
	CrystalFreqKhz = (s32)((pExtra->CrystalFreqHz + 500) / 1000);
...
//
/*
s32 Ref_clk = 26000;   // Reference clock frequency(kHz).
s32 Freq = 590000; // RF Frequency (kHz)
s32 bandwidth = 8000;  //RF channel bandwith (kHz)
*/
//
...
s32 filtercal();
s32 Qpeak();
s32 PLL(s32 Ref_clk, s32 Freq);
s32 LNAfilter(s32 Freq);
s32 IFfilter(s32 bandwidth, s32 Ref_clk);
s32 freqband(s32 Freq);
s32 DCoffLUT();
s32 DCoffloop();
s32 commonmode();
s32 GainControlinit();  
...
\****************************************************************************/
/*
s32 filtercal(TUNER_MODULE *pTuner)
{
  //writearray[0] = 1;
...
*
\****************************************************************************/
s32 E4000_gain_freq(TUNER_MODULE *pTuner, s32 Freq)
{
	u8 writearray[5];
	s32 status;
...
\****************************************************************************/
/*
s32 commonmode(TUNER_MODULE *pTuner)
{
     //writearray[0] = 0;  
...
//
/*
s32 Ref_clk = 20000;   // Reference clock frequency(kHz).
s32 Freq = 590000; // RF Frequency (kHz)
s32 bandwidth = 8;  //RF channel bandwith (MHz)
*/
//
// API defined variable definitions
//s32 VCO_freq;
...
//	SigDel=65536 * 1000 * (VCO_freq - intVCOfreq) / Ref_clk;
	...
	SigDel2 = SigDel / 256;
	//printf("\nSigdel2=%d", SigDel2);
	writearray[2] = (u8)SigDel2;
	SigDel3 = SigDel - (256 * SigDel2);
	//printf("\nSig del3=%d", SigDel3);
	writearray[1]= (u8)SigDel3;
	writearray[3]=(u8)0;
