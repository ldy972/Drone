...
*/
s32 dtmb_demod_addr_8bit_default_SetRegPage(DTMB_DEMOD_MODULE *pDemod, u64 PageNo)
{
	...
	)
{
	...
	for(i = 0; i < ByteNum; i++)
	{
		...
		WritingBuffer[0] = (u8)(RegStartAddr + i);
...
error_status_set_demod_registers:
	...
	u64 PageNo = pDemod->CurrentPageNo;
	...
	for(i = 0; i < ByteNum; i++)
	{
		...
		// Set demod register bytes with writing buffer.
			if(write_demod_register(d, DeviceAddr, PageNo,  RegStartAddr+i,  (u8*)pWritingBytes+i, LEN_1_BYTE))
			goto error_status_set_demod_registers;	
...
{
	u64 CurrentPageNo;		// temp, because page register is write-only.
	...
	)
{
	...
	u8 WritingBuffer[LEN_4_BYTE];
	u8 ReadingBuffer[LEN_4_BYTE];
 ...
 * @see   DTMB_DEMOD_FP_GET_REG_BITS
 */
s32 dtmb_demod_addr_16bit_default_GetRegBits(DTMB_DEMOD_MODULE *pDemod, s32 RegBitName, u64 *pReadingValue)
{
	u16 RegStartAddr;
	u8 Msb;
	u8 Lsb;
 ...
 */
void dtmb_demod_default_GetDemodType(DTMB_DEMOD_MODULE *pDemod, s32 *pDemodType)
{
 ...
 */
void dtmb_demod_default_GetDeviceAddr(DTMB_DEMOD_MODULE *pDemod, u8 *pDeviceAddr)
{
 ...
 */
void dtmb_demod_default_GetCrystalFreqHz(DTMB_DEMOD_MODULE *pDemod, u64 *pCrystalFreqHz)
{
 ...
 */
s32 dtmb_demod_default_GetIfFreqHz(DTMB_DEMOD_MODULE *pDemod, u64* pIfFreqHz)
{
 ...
 */
s32 dtmb_demod_default_GetSpectrumMode(DTMB_DEMOD_MODULE *pDemod, s32 *pSpectrumMode)
{
